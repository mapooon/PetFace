...
class CenterLoss(torch.nn.Module):
    ...
    def __init__(self, in_features, out_features):
        ...
        self.centers = torch.nn.Parameter(torch.randn(self.out_features, self.in_features))
    ...
    def forward(self, x, labels):
        ...
        labels = labels.unsqueeze(1).expand(batch_size, self.out_features)
        mask = labels.eq(classes.expand(batch_size, self.out_features))
...
def main(args):
    ...
    if cfg.optimizer == "sgd":
        fc = CenterLoss(in_features=cfg.embedding_size, out_features= num_classes).cuda()
    ...
    if cfg.resume:
        ...
        fc.load_state_dict(dict_checkpoint["state_dict_center_fc"])
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, local_labels) in enumerate(train_loader):
            ...
            loss: torch.Tensor = fc(local_embeddings, local_labels)#, opt)
            opt.zero_grad()
        ...
        if cfg.save_all_states:
            checkpoint = {
                ...
                "state_dict_softmax_fc": fc.module.state_dict(),
